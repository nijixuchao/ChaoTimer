...
- (UIView *)createContainerView
{
    ...
    gradient.colors = [NSArray arrayWithObjects:
                       ...
                       nil];
     */
    gradient.colors = [NSArray arrayWithObjects:(id)[[UIColor whiteColor] CGColor],(id)[[UIColor whiteColor] CGColor],(id)[[UIColor whiteColor] CGColor], nil ];
    [gradient setOpacity:0.95f];
    CGFloat cornerRadius = kCustomIOS7AlertViewCornerRadius;
    gradient.cornerRadius = cornerRadius;
    [dialogContainer.layer insertSublayer:gradient atIndex:0];
    dialogContainer.layer.cornerRadius = cornerRadius;
    dialogContainer.layer.borderColor = [lineColor CGColor];
...
- (void)addButtonsToView: (UIView *)container
{
    ...
    // add a vertical line, by Jichao Li
    UIView *lineView = [[UIView alloc] initWithFrame:CGRectMake(container.bounds.size.width/2, container.bounds.size.height - buttonHeight - buttonSpacerHeight, kCustomIOS7AlertViewDefaultButtonSpacerHeight, buttonHeight)];
    lineView.backgroundColor = lineColor;
    ...
    for (int i=0; i<[buttonTitles count]; i++) {
        ...
        if ([closeButton.titleLabel.text isEqualToString:NSLocalizedString(@"yes", NULL)]){
